    $(function () {
      var c=0;
      var a=0;
      var cl=$('#texto1').html("Llevas: ");
      var cl2=$('#texto3').html("Llevas: ");
      var v = document.getElementsByTagName("audio")[0];
      var v1 = document.getElementsByTagName("audio")[1];
      var v2 = document.getElementsByTagName("audio")[2];
      $('#drag1').draggable({
        start: function () {
          v.play();
          console.log('cdcd');
          this.css('-webkit-transform', 'scale(2)');
        },
        stop: function () {
          this.css('-webkit-transform', 'scale(1)');
        }
      });
      $('#drag2').draggable({
        start: function () {
          v.play();
           this.css('-webkit-transform', 'scale(2)');
        },
        stop: function () { 
         this.css('-webkit-transform', 'scale(1)');
        }
      });
      $('#drop').droppable({
        drop: function (e, dragEl, dropEl) {
          a+=1;
          cl.html("Llevas: "+a + "  pollito")
          dragEl.removeAttr('id');
          dragEl.removeAttr('src');
          next();
        }
      });
      $('#drop2').droppable({
        drop: function (e, dragEl, dropEl) {
          c+=1;
          cl2.html("Llevas: "+c + "  pollito")
          dragEl.removeAttr('id');
          dragEl.removeAttr('src');
          next();
        }
      });
      function next() {
        if (a==1 && c==1) {
          v1.play();
          $('#image').attr('src','../img/carga.gif');
          setTimeout(function(){acierto()},2000);
        }
        if (a>1) {cl.html("Llevas: "+a+" pollitos")}
          if (c>1) {cl2.html("Llevas: "+c+" pollitos")}
        if (a==2 || c==2) { 
          $('#image').attr('src','../img/carga.gif');
          setTimeout(function(){error()},2000);
        }
      }
     function acierto(){
        $('#image').removeAttr('src')
        alert("hay 1 Pollito para cada Gallina: correcto");
        location.href='reparto2.html';
        v2.play();
      }
      function error(){
        v2.play();
        $('#image').removeAttr('src')
        alert('Algo hiciste mal vuelve a intentarlo')
        
        v2.play();
        location.reload();
      }
    });