$(function () {
    ...
    function resta(num, num2, den, den2) {
        var arriba = (num * den2) - (num2 * den);
        var abajo = (den * den2);
        if (arriba < 0) {//si es menor que 0
            arriba = arriba * (-1);//convertimos a positivo
            alert("es negativo");
            simplificar(arriba, abajo);
        } else {//si 
            if (arriba == 0) {//es igual a 0
                cero(); //mandar a visualizar 0 y terminar
            } else {
                simplificar(arriba, abajo);
            }
        }
    ...
    function cero() {
        img.attr('src', '../fracciones/cero.png');
        document.getElementById("resultado").style.display = 'block';
        cl.html("El numero es cero");
    }
